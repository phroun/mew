"""kittytk - the KittyTK display-protocol client, in Python.

A pure-protocol client: it links nothing of the rendering side and speaks
the identical wire language the Go client does, so a Python app drives the
same display host (kittytk-tui / kittytk-sdl) as any other.
"""

from .protocol import (
    ArgInfo,
    CallInfo,
    Event,
    EventInfo,
    FlagState,
    ParseError,
    PropInfo,
    TypeInfo,
    Value,
    ValueKind,
    Vocabulary,
    decode_vocabulary,
    parse,
    parse_event,
    quote,
    quote_blob,
)
from .query import (
    ANSWER_VERB,
    Answer,
    CHANGE_ADDED,
    CHANGE_ALTERED,
    CHANGE_REMOVED,
    CHANGE_REPLACED,
    Fields,
    Filter,
    QUERY_VERB,
    QueryError,
    RESULT_COMPLETE,
    RESULT_VERB,
    PLACE_VERB,
    STOP_FILLED,
    STOP_JOINED,
    STOP_EXHAUSTED,
    DataSetDescriptor,
    SortLevel,
    Scope,
    Result,
    Complete,
    encode_answer,
    encode_sort,
    STALE_VERB,
    Stale,
    encode_stale,
    parse_answer,
    parse_fields,
    parse_stale,
    parse_scope,
    parse_result,
    parse_filter,
    parse_sort,
    parse_descriptor,
)
from .client import (
    ASK_DARK,
    ASK_DESKTOP,
    Blob,
    Button,
    CACHE_MARK,
    Checkbox,
    Conn,
    DO_RELAY,
    EVENT_RELAY,
    Fill,
    Handle,
    Label,
    Query,
    STORE_BLOB,
    STORE_ERROR,
    STORE_GONE,
    Selector,
    Source,
    Store,
    TextInput,
    UI,
    Window,
    default_endpoint,
    default_socket_path,
    dial,
    dial_solo,
)

__all__ = [
    "Event", "FlagState", "ParseError", "Value", "ValueKind",
    "ArgInfo", "CallInfo", "EventInfo",
    "PropInfo", "TypeInfo", "Vocabulary", "decode_vocabulary",
    "parse", "parse_event", "quote", "quote_blob",
    "Fields", "Filter", "DataSetDescriptor", "SortLevel", "Scope", "Result", "Complete",
    "Query", "QueryError",
    "Answer", "ANSWER_VERB", "parse_answer", "encode_answer",
    "Stale", "STALE_VERB", "parse_stale", "encode_stale",
    "CHANGE_ADDED", "CHANGE_REMOVED", "CHANGE_REPLACED", "CHANGE_ALTERED",
    "parse_descriptor", "parse_scope", "parse_result", "parse_fields", "parse_filter",
    "parse_sort",
    "encode_sort", "QUERY_VERB", "RESULT_VERB", "PLACE_VERB", "RESULT_COMPLETE",
    "STOP_FILLED", "STOP_JOINED", "STOP_EXHAUSTED", "Source",
    "Button", "Checkbox", "Conn", "Handle", "Label", "Selector",
    "TextInput", "UI", "Window",
    "Blob", "Store", "CACHE_MARK",
    "STORE_BLOB", "STORE_GONE", "STORE_ERROR",
    "ASK_DARK", "ASK_DESKTOP", "DO_RELAY", "EVENT_RELAY",
    "default_endpoint", "default_socket_path", "dial", "dial_solo",
]
