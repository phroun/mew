# Resource scheme architecture: `profile://` and `app://`
...
- **`app://`** — a single Application's private sandbox.
...
|---|---|---|---|
| `app:///path` | empty | **the calling Application's own** sandbox | auto-granted |
| `app://name/path` | `name` | another Application's sandbox | brokered |
...
For `app://`, "someone else" is another Application. For `profile://`, it is
...
assign the single-slash form (`app:/x`) a separate meaning — it collapses to the
empty-authority form in stock parsers, so treat it as an alias of `app:///x`, or
reject it.
...
- `app://name/…` is the **canonical** way to name any sandbox — your own or a
  sibling's.
...
The Desktop resolves `profile://` and `app://`; Applications never do their own
path math and never receive a real filesystem path back. The scheme is
**opaque** by design: an Application addresses `app:///config` and the Desktop
...
1. **Application capability** — governs whether an Application may leave its own
   sandbox: `app:///…` (its own) is auto-granted; `app://sibling/…` and
...
`profile://` and `app://` address **stored** resources. An Application may also
...
`app://name/…`, never through another Application's private scheme.

This keeps a clean separation:

- **stored, file-backed, user-overridable** → `profile://` / `app://` (Desktop-owned);
...
Each scheme's authority is a **single axis** — `app://` names an Application,
...
- `app://` is **implicitly current-user** for now (it always means this user's
  Applications); and
- RFC 3986's authority is `userinfo@host:port` — richer than one token — so the
  cross-product has structural room (`app://name@user/…` or similar) if and when
