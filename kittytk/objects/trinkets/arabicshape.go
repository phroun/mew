...
//
// The same table and logic are intended for purfecterm's gtk and qt widgets
// (see patches/purfecterm/); keep the copies in sync.
...
// shapeArabicVisual resolves the glyph a cell should DRAW for base character c
...
func shapeArabicVisual(left, c, right rune) (glyph rune, suppress bool) {
	f, ok := arabForms[c]
	if !ok {
		return c, false
	}
