...
func TestViewportPinsAcrossEditAbove(t *testing.T) {
	...
	// Insert three lines at the very top (as a sibling viewport/edit would).
...
// Two viewports on the SAME buffer scroll independently: each has its own anchor.
func TestTwoViewportsIndependentViewports(t *testing.T) {
...
// viewport_clone opens a second viewport on the SAME buffer. Editing through one
// viewport's caret slides the other viewport's caret (both are live garland
// cursors), so switching between them lands on the right line — the payoff of
// per-viewport caret cursors.
func TestViewportCloneSharedBufferCaret(t *testing.T) {
	e, w1 := newTestEditor(t, "aaa\nbbb\nccc\nddd\n")
	w1.SetCursorPos(viewport.Position{Line: 3, Rune: 0}) // on "ddd"

	if !e.cloneCurrentViewport() {
		t.Fatal("viewport_clone failed")
	}
	w2 := e.ViewportManager.GetFocusedViewport()
	if w2 == nil || w2.ID == w1.ID {
		t.Fatal("clone should be a new, focused viewport")
	}
...
func TestPageUpDownClampAtEdges(t *testing.T) {
	...
	w.SetCursorPos(viewport.Position{Line: 5, Rune: 0})
	...
	w.SetCursorPos(viewport.Position{Line: last - 4, Rune: 0})
	e.PawScript.ExecuteAsync("go_page_next")
...
func TestPageDownCapsOneBlankLineAtEnd(t *testing.T) {
	...
	w.SetCursorPos(viewport.Position{Line: 90, Rune: 0})
	w.SetViewTop(75)
	e.PawScript.ExecuteAsync("go_page_next")
...
func TestPageDownNoRewindWhenDeeper(t *testing.T) {
	...
	w.SetCursorPos(viewport.Position{Line: 95, Rune: 0})
	w.SetViewTop(82)
	e.PawScript.ExecuteAsync("go_page_next")
	if w.ViewState.ViewOffsetY < 82 {
