...
// The mouse-pointer affordance moved from a per-motion push to a render-time
// I-beam REGION (see pointershape_test.go: TestPointerRegionPublished,
// TestPointerRegionFollowsPromptFocus, TestPointerRegionExcludesBrowseButtons),
// so there is no longer a per-mouse-move pointer-shape hook to exercise here.
