...
// (line, rune) — so the viewport setters clamp the rune to the line's length.
func TestSetCursorClampsRunePastLineEnd(t *testing.T) {
	...
	w.SetCursorPos(viewport.Position{Line: 0, Rune: 5}) // end of "hello"
	...
	w.SetCursorPos(viewport.Position{Line: 1, Rune: 0}) // "world"
	w.SetCursorRune(99)
...
func TestSetCursorStaysConsistentPastEnd(t *testing.T) {
	...
	w.SetCursorPos(viewport.Position{Line: 1, Rune: 5})
	cp := w.CursorPos()
	if cp.Line != 1 {
