...
func TestMarkContextHebrewPoint(t *testing.T) {
	...
	w.SetCursorPos(viewport.Position{Line: 0, Rune: 2}) // just past the sin dot
	e.performRender()
	if !strings.Contains(w.Context, "hebrew point sin dot") {
		t.Errorf("context should name the sin dot, got %q", w.Context)
	}
	w.SetCursorPos(viewport.Position{Line: 0, Rune: 3}) // just past the qamats
	e.performRender()
...
func TestMarkContextOrdinaryRune(t *testing.T) {
	e, w, _ := newRenderedEditor(t, "éx\n")
	w.SetCursorPos(viewport.Position{Line: 0, Rune: 1}) // just past the plain 'e'
	e.performRender()
